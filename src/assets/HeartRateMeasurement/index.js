...
export default class HeartRateTool extends BaseAnnotationTool {
  constructor(props = {}) {
    const defaultProps = {
      name: 'HeartRate',
  ...
  updateCachedStats = (image, element, data) => {
    const { ultrasonicX, ultrasonicY } = this.getPixelSpacing(image);
    let unit = 'pixels';
    if (ultrasonicY && ultrasonicX) {
      unit = 'bpm';
    }
    const time = Math.abs(data.handles.end.x - data.handles.start.x) * (ultrasonicX || 1);

    // 两个心动周期间的计算公式为 (2/time)*60
    const length = ((2 / time) * 60).toFixed(3);
