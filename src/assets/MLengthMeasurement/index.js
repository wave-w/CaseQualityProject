...
export default class MLengthTool extends BaseAnnotationTool {
  constructor(props = {}) {
    const defaultProps = {
      name: 'MLength',
  ...
  updateCachedStats = (image, element, data) => {
    ...
    const time = Math.abs((data.handles.end.y - data.handles.start.y) * (ultrasonicY || 1));

    const length = time.toFixed(3);
